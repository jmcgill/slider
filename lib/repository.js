...
class Repository {
  ...
  init() {
    ...
    if (!fs.existsSync(this._localPath())) {
      ...
      return this._cloneRepository().then(() => {
    ...
    return this.repo
      .remote_fetchAsync('origin')
      .then(() => _.repo.checkoutAsync('master'))
      .then(() => _.repo.resetAsync('origin/master', { hard: true }))
      .then(() => _.repo.current_commitAsync())
      .then(commit =>
  ...
  commit(branchName) {
    ...
    return _.statusToRemote(branchName).then((status) => {
      if (status.length == 0) {
      ...
      return _.repo.checkoutAsync(branchName).then(() => _.repo.commitAsync('Commiting bulk modification', {
        all: true,
      }));
    });
  }
  ...
  statusToRemote(remote) {
    const _ = this;
    return this.repo.addAsync('.').then(() => _.repo.diffAsync('origin/master', '', {
  ...
  branch(name) {
    ...
    return _.repo.branchesAsync().then((branches) => {
      ...
      if (!found) {
        ...
        return _.repo.create_branchAsync(name).then(() => _.repo.checkoutAsync(name));
      }
  ...
  _cloneRepository() {
    ...
    return new Promise(((resolve, reject) => {
      ...
      gift.clone(_.url, localPath, 5, null, (err, repository) => {
        if (err) {
          reject(err);
        }
